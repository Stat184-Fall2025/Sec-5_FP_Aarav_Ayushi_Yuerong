...
## Derparture and Arrival Rates Summary Tables
...
delay_rates_all <- Flights %>%
  mutate(
    dep_delayed = dep_delay > 15,
    arr_delayed = arr_delay > 15
  ) %>%
  group_by(origin) %>%
  summarise(
    dep_delay_rate = mean(dep_delayed),
    arr_delay_rate = mean(arr_delayed),
    n = n(),
    .groups = "drop"
  )
...
total_delay_rates <- delay_rates_all %>%
  summarise(
    origin = "Across All Three Airports",
    dep_delay_rate = round(mean(dep_delay_rate, na.rm = TRUE), 4),
    arr_delay_rate = round(mean(arr_delay_rate, na.rm = TRUE), 4),
    n = sum(n, na.rm = TRUE)
  )
...
This table allows us to see the results in a more organized way by comparing delay rates across JFK, EWR, and LGA, plus an overall summary across all three airports.
...
```{r}
#| author:  "Aarav Johri and Ayushi Bisaria"
#| reviewer: "Yuerong Shu"
#| echo: false
#| warning: false
#| message: false
Flights_cat <- Flights %>%
...
```{r}
#| author: "Yuerong Shu and Ayushi Bisaria"
#| reviewer: "Aarav Johri"
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| author: "Aarav Johri"
#| reviewer: "Yuerong Shu and Ayushi Bisaria
#| echo: false
#| warning: false
#| message: false
